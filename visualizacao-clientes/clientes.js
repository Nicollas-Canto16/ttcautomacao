const sunIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-brightness-high-fill" viewBox="0 0 16 16">
...
function toggleTodos(checkboxTodos) {
  // Pega todos os checkboxes individuais da tabela
  const checkboxes = document.querySelectorAll(".checkbox-cliente");
  // Marca ou desmarca todos conforme o estado do checkbox do cabeçalho
  checkboxes.forEach(cb => cb.checked = checkboxTodos.checked);
}


function confirmarExclusao(id, nome) {
  // Mostra um alerta de confirmação com o nome do cliente
  if (confirm(`Tem certeza que deseja excluir o cliente "${nome}"?`)) {
    // Se confirmou, redireciona para o arquivo PHP que faz a exclusão
